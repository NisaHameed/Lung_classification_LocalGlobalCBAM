...
class Trainer:
    ...
    def report(self, all_losses, all_acc, valid_acc, epoch, duration):
        ...
        msg = 'epoch {epoch}: loss {loss:.3f} Tr Acc {tr_dist:.2f} Val Acc {va_dist:.2f} AUC {auc:.2f} duration {duration:.2f}'
        
        print('epoch '+ str(epoch) + 'loss  :  '  + str(loss)  + 'Tr Acc : '  +  str(tr_dist) + 'val acc : ' + str(va_dist)  + 'AUC: ' + str(auc) + '\n')
        self.log += msg + '\n'
    ...
    def run(self):
        ...
        with open(path.join('results','{self.name}.txt'),'w') as f:
            f.write(self.log)
